...
//     const result= await geminiResponse(command,assistantName,userName)
//     console.log("3. Gemini result:", result)

//     const jsonMatch=result.match(/{[\s\S]*}/)
//     console.log("4. JSON match:", jsonMatch)
...
//                       });
//      default: return res.status(400).json({response:"I didn't understand that command."})          
//    }
...
export const askToAssistant = async (req, res) => {
  try {
    ...
    if (!command) {
      return res.status(400).json({ response: "No command received" })
    }
    ...
    const result = await geminiResponse(command, assistantName, userName)
      if (!result) {
    return res.status(500).json({ response: "Gemini API failed to respond." })
}
    const jsonMatch = result.match(/{[\s\S]*}/)
    if (!jsonMatch) {
      return res.status(400).json({ response: "Sorry, I can't understand" })
